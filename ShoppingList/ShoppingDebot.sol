...
abstract contract ShoppingDebot is Debot {
    ...
    function savePublicKey(string value) public {
        ...
        if(status){
            p_masterPubKey = res;
            Terminal.print(0, "Check if you already have a purchase list...");
            TvmCell depState = tvm.insertPubkey(p_StateInit, p_masterPubKey);
            p_address = address.makeAddrStd(0,tvm.hash(depState));
            Terminal.print(0, format( "Your Shopping List contract address is {}", p_address));
    ...
    function waitBeforeDeploy() public  {
        Sdk.getAccountType(tvm.functionId(checkIfStatusIs0), p_address);
    }
   ...
   function setTotal(PurchaseTotal stat) public {
    ...
    function _getTotal(uint32 answerId) private view{
        optional(uint256) none;
